...
export type ILanguageFiles = Record<
  string,
  {
...
class FileOperator {
  ...
  public async getFileContent(
    ...
    if (fs.existsSync(filePath)) {
      ...
      const exportType = fileContent.includes('module.exports') ? 'CJS' : 'ESM'
      try {
  ...
  public async getConfig(getNew?: boolean): Promise<ILoaderOptions> {
    ...
    const { content: config } = await this.getFileContent(configPath) || {};
  ...
  public async initMessage(path: string, clear?: boolean) {
    ...
    if (!this.messages) {
      const { content: message} = await this.getFileContent(path) || {}
  ...
  // 写入翻译文件
  public async writeMessages(
    outputDir: string,
    localeFile: string,
    exportType: string = "ESM",
    messages: MessageType = null
  ) {
  ...
  // 初始化设置翻译文件内容
  public setTotalTranslateMessages(
    languageFiles: ILanguageFiles,
    clearInexistence: boolean = false
  ) {
    this.languageFiles = Object.fromEntries(
      Object.entries(languageFiles).map(([key, value]) => {
        let { messages } = value
        ...
        if (clearInexistence) {
          ...
          for (const k in messages) {
            if (this.getMessage(k)) {
              newMessages[k] = messages[k] as string;
            } else {
              needUpdate = true;
            }
          }
  ...
  public saveLanguageFiles(outputDir: string, exportType?: string) {
    for (const key in this.languageFiles) {
      const targetFile = this.languageFiles[key]!;
      if (targetFile.update || targetFile.exportType !== exportType) {
  ...
  public loadTranslationCache(outputDir: string): ITranslationLog {
    ...
    try {
      if (fs.existsSync(cachePath)) {
        this.translationLog = JSON.parse(
