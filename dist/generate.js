...
import fs from 'fs';
import { a as getDefaultExportFromCjs, r as requireSrc, f as fileOperator, b as requestPool, S as ScriptLoader, g as getVueModule, T as TemplateLoader } from './script-loader-B4WC5WYH.js';
   ...
   */
    ...
    getValueFromText(content) {
        try {
            ...
            const matchResult = markdownJsonMatch?.[1];
...
const getTranslatePrompt = (sourceTextList, targetLanguages) => {
    const userPrompt = `
    请将以下中文文本翻译成指定的语言。
    中文文本列表：
      ${sourceTextList}
...
// 加载所有需要翻译的语言文件
const getLanguagesFiles = async (options) => {
    const languagesFileContent = {};
...
// 为每个键收集需要翻译的语言
async function getkeyLanguageMap(options, check = false) {
    const messages = fileOperator.messages;
    let useCache = false;
    const keyLanguageMap = {};
    if (!messages) {
        return {
            keyLanguageMap,
            useCache
...
// 按需要翻译语言组合分组
const getLanguageGroups = (keyLanguageMap) => {
...
const getPrompt = (keyItems, languageNames, prompt = '') => {
...
const getTranslatePromise = (analyzePromise, chunk, count) => {
    return new Promise((resolve, reject) => {
...
const executeTranslate = async (options) => {
    try {
        ...
        for (const chunk of chunks) {
            ...
            const keyItems = keys.map(key => ({ id: key, text: messages[key] }));
            const languageNames = languages.map(lang => options.translateList.find(item => item.fileName === lang)?.name);
...
chalk.level = 3;
const generateVueFile = (path, options) => {
    try {
...
const generateI18nFiles = async (options, writeMessage = true) => {
